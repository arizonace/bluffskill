...
ReferencePlayer::ReferencePlayer(std::string name, ReferencePlayerProfile profile)
    ...
    if (!valid(profile_.riskTolerance) || !valid(profile_.optimism) || !valid(profile_.variability)) {
        throw std::invalid_argument("reference-player profile values must be between 0 and 1");
    }
}

ReferenceDecision ReferencePlayer::chooseResponse(const TableView& privateView, std::mt19937_64& random) const {
    ...
    if (player == privateView.players.end() || !player->acting || !privateView.legalActions) {
        throw std::invalid_argument("reference player was asked to act without a private legal table view");
    }
    ...
    throw std::logic_error("reference player has no legal response");
}
