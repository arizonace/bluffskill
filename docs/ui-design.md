...
The eight seats use a fixed semantic layout: three top, three bottom, left, and right. A seat presents name, stack, circular dealer and blind buttons touching its inward edge (side by side when both apply), a folded-hands marker inside its inward edge after folding, the latest action in a white box on the player's right, and a clear local-player ring. A busted player's seat is dark gray. The center presents the pot, side pots, current betting-round wager, and community cards. Community and local hole cards render as compact rank-and-suit card faces with a visible margin before any hand-category description. At a completed hand, each winner has a yellow two-line result box on the player's left: total chips awarded from the pot and net hand result. The client does not reconstruct per-pot allocation or draw chip stacks. The requesting player sees their own ranking description even after folding or a non-showdown hand. The clickable dealer button starts the next hand immediately; otherwise the client requests it after the configured deal clock (20 seconds by default), or after the uninterrupted dealer delay when the local player has been eliminated.

The top status area presents server, competition, and table selectors in one row. A second row shows remaining players, a visible next-deal countdown, and a Deal Now button enabled only during that countdown. When the human player can act, the action panel shows the configured turn countdown and a Pause/Play control. On expiry, the client submits Check if still legal, otherwise Fold; the server continues to validate the command. The wager composer sits below the action buttons: it accepts an editable total commitment and 25/100/500/1000 chip increment/decrement controls.
