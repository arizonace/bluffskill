...
Both applications read and create the shared INI-style configuration at `~/.config/azonelayer/blindskill/blindskill.conf`. It stores the player and deal clock values, blind-level hand/minute thresholds, default human player name, three preferred server ports, and client auto-connect preference. The server raises the blind level before a new hand after either configured threshold is reached. Do not encode state in color alone; provide text labels and accessible names.
